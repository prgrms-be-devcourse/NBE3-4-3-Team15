...
export default function Mine() {
  ...
  const getUserProfile = async () => {
    try {
      ...
      if (response.data?.data) {
        const { id, username, email, nickname, gender, birth } =
          response.data.data;
  ...
  const goToChallengePage = () => {
    if (username) {
      router.push(`/member/mine/challenge`);
    } else {
      alert("유저 정보가 없습니다.");
    }
  };
  ...
  return (
    <div className="profile-container">
      {userProfile ? (
        <div className="profile-card">
          ...
          <div className="button-group">
            {isEditing ? (
              <>
                <button className="save-btn" onClick={modify}>
                  저장
                </button>
                <button className="cancel-btn" onClick={edit}>
                  취소
                </button>
              </>
            ) : (
              <>
                ...
                <button className="password-btn" onClick={passwordEdit}>
                  비밀번호 변경
                </button>
                <button
                  className="favorite-btn"
                  onClick={goToFavoriteBooksPage}
                >
                  찜한 도서 목록
                </button>
                <button className="follow-btn" onClick={goToFollowPage}>
                  팔로우 관리
                </button>
                <button className="follow-btn" onClick={goToChallengePage}>
                  내 챌린지
                </button>
                <button
      ...
      >
        ...
        />
        <button className="delete-btn" onClick={quit}>
          탈퇴하기
        </button>
      </Modal>
